...
func createAgent(ctx context.Context, cmd *cli.Command) error {
	...
	resp, err := agentsClient.CreateAgent(ctx, os.DirFS(workingDir), secrets, regions, excludeFiles, nil)
	if err != nil {
...
func deployAgent(ctx context.Context, cmd *cli.Command) error {
	...
	excludeFiles := []string{fmt.Sprintf("**/%s", config.LiveKitTOMLFile)}
	if err := agentsClient.DeployAgent(ctx, agentId, os.DirFS(workingDir), secrets, excludeFiles, nil); err != nil {
		if twerr, ok := err.(twirp.Error); ok {
