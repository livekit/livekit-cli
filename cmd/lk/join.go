	...
	JoinCommands = []*cli.Command{
		{
			...
			Flags: []cli.Flag{
				...
				&cli.BoolFlag{
					...
					Usage:  "When publishing H.264/H.265, attach the LKTS user timestamp trailer to each encoded frame",
					Hidden: true,
				},
				&cli.FloatFlag{
					Name:  "fps",
...
func _deprecatedJoinRoom(ctx context.Context, cmd *cli.Command) error {
	...
	if cmd.StringSlice("publish") != nil {
		...
		for _, pub := range cmd.StringSlice("publish") {
			...
			if err = handlePublish(room, pub, fps, h26xStreamingFormat, cmd.Bool("publish-user-timestamp"), onPublishComplete); err != nil {
				return err
			}
		}
	}
