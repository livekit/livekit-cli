...
func (t *LoadTester) Start() error {
	...
	l := logger.GetLogger()
	l = l.WithValues("room", t.params.Room, "participant", identity)
	lksdk.SetLogger(l)
	...
	for i := 0; i < 10; i++ {
		...
		l.Warnw("failed to join, retrying...", err)
	...
	l.Infow("participant joined", "timeToJoinMs", timeToJoin.Milliseconds())
