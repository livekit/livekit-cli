# This Dockerfile creates a production-ready container for a LiveKit Node.js agent using pnpm
...
# Set pnpm environment variables for optimal performance
ENV PNPM_HOME="/pnpm"
...
# This keeps only the packages needed for runtime
RUN pnpm prune --prod
...
#    - For development: CMD ["pnpm", "run", "dev"]
#
...
#    - pnpm uses symlinks; ensure they're preserved in COPY
#
# 3. "EACCES: permission denied" errors:
#    - Add a non-root user (see example above)
#    - Ensure files have correct permissions
#    - pnpm global store may need permission adjustments
#
# 4. Large image sizes:
#    - Use node:20-alpine instead of node:20-slim for smaller base
#    - Ensure .dockerignore excludes unnecessary files
#    - Use pnpm prune --prod after build
#    - Consider using pnpm deploy for production-only installs
#
# 5. Slow builds:
#    - Use Docker BuildKit: DOCKER_BUILDKIT=1 docker build
#    - Order COPY commands from least to most frequently changed
#    - Copy package.json and pnpm-lock.yaml before source code for better caching
#    - Consider using pnpm fetch to pre-download packages
#
...
#
# 8. pnpm workspace issues:
#    - If using workspaces, copy pnpm-workspace.yaml
#    - Use --shamefully-hoist if packages expect flat node_modules
#    - Consider using pnpm deploy for single-package deployment
#
